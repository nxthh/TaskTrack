...
void TaskManager::editTask(int id, const string &owner, bool isAdmin) {
  ...
  if (!found)
    cout << "   Task ID " << id << " not found or access denied.\n";
}
...
void TaskManager::searchTasks(const string &keyword, const string &owner,
                              bool isAdmin) const {
...
void TaskManager::filterByStatus(const string &status, const string &owner,
                                 bool isAdmin) const {
  auto list = filterOwner(tasks, owner, isAdmin);
...
void TaskManager::restoreTask(int id, const string &owner, bool admin) {
  auto it = find_if(trash.begin(), trash.end(), [&](const Task &t) {
    return t.getId() == id && (admin || t.getAssignee() == owner);
  });

  if (it == trash.end()) {
    cout << "   Error: Task not found in trash.\n";
    return;
  }
  ...
  // ── Auto-sort active tasks by ID after restore ──────────
  ...
  saveToFile();
  saveTrash();
  cout << "   Task restored and re-inserted in order.\n";
}
